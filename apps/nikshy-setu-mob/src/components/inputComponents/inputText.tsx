...
export const InputText: React.FC<Props> = ({
  ...
  return (
    <React.Fragment>
      ...
      >
        <Row >
          <Text style={uiStyles.InputTextLabel}>
            {" "}{label} {touched && <Text style={fontStyles.InputTextErrorText}>*</Text>}
          </Text>
      ...
      {error && touched && <Text style={fontStyles.InputTextErrorText}>{error}</Text>}
    </React.Fragment >
  );
};
...
const AnimatedText = ({ option, isSelected }) => {
  ...
  return (
    <Animated.Text style={[styles.text, { backgroundColor: bgColor, color: color }]}>
      {option}
    </Animated.Text>
  );
};
    ...
    <React.Fragment>
      ...
      >
        ...
        <Row style={{ justifyContent: "space-between", alignItems: "center", marginVertical: 12, }} >
          <Text style={uiStyles.TextInputInputComponent}>
            {placeholder}
          </Text>
          <Image source={require('../../assets/images/drpdwnIC.png')} style={{ height: 7, width: 13, marginHorizontal: 10 }} />
        </Row>

        {showOption && <View style={styles.row}>
